...
public class QuizRoomController {
	...
	public ResponseEntity<ApiResponse<Long>> createQuizRoom(@RequestBody CreateQuizRoomRequest request) {
		...
		if (memberId <= 0) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.failure(HttpStatus.BAD_REQUEST, "멤버ID가 유효하지 않습니다."));
		}

		//2. 새로운 채팅방 생성하기
		long quizRoomId = quizRoomService.createQuizRoom();
        if (quizRoomId <= 0) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.failure(HttpStatus.NOT_FOUND, "새로운 퀴즈방 생성에 실패했습니다"));
        }
		...
		if (!isOk) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.failure(HttpStatus.NOT_FOUND, "호스트를 퀴즈방에 넣는 과정이 실패했습니다."));
		}
	...
	public ResponseEntity<ApiResponse<String>> joinQuizRoom(@RequestBody JoinQuizRoomRequest request) {
        ...
        //퀴즈방 기본 검증
        if (roomId <= 0) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.failure(HttpStatus.BAD_REQUEST, "퀴즈방ID가 유효하지 않습니다."));
        }
        //퀴즈방이 존재하는지 확인
        boolean roomExists = quizRoomService.existsQuizRoom(roomId);
        if (!roomExists) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.failure(HttpStatus.NOT_FOUND, "존재하지 않는 퀴즈방입니다."));
		...
		if (!isOk) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.failure(HttpStatus.NOT_FOUND, "퀴즈방 입장에 실패하였습니다."));
        }
    ...
    public ResponseEntity<ApiResponse<List<QuizRoom>>> getQuizRoomList() {
        ...
        if (quizRooms == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.failure(HttpStatus.BAD_REQUEST, "퀴즈방 정보를 가져오지 못했습니다"));
        }
        else if (quizRooms.isEmpty()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.failure(HttpStatus.BAD_REQUEST, "생성된 퀴즈방이 없습니다."));
        }
    ...
    public ResponseEntity<ApiResponse<List<QuizRoomMember>>> getQuizRoomMembers(@PathVariable long roomId) {
        if (roomId <= 0) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.failure(HttpStatus.BAD_REQUEST, "퀴즈방 ID가 유효하지 않습니다."));
        }
        ...
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.failure(HttpStatus.NOT_FOUND, "참가자가 없습니다."));
    ...
    public ResponseEntity<ApiResponse<String>> deleteQuizRoom(@PathVariable long roomId) {
        ...
        if (!isOk) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.failure(HttpStatus.BAD_REQUEST, "퀴즈방 삭제에 실패했습니다."));
        }
        return ResponseEntity.status(HttpStatus.OK).body(ApiResponse.success(HttpStatus.OK, "성공적으로 퀴즈방이 삭제되었습니다."));
    }
